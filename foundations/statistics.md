...
$\text{True positive rate}~:= \frac{\text{correctly classified positives}}{\text{total actual positives}} = \frac{\text{FP}}{\text{FP} + \text{TN}}$

### False positive rate \rightarrow Want 1.0
...
$\text{Precision} := \frac{\text{correctly classified positives}}{\text{everything classified as positive}} = \frac{\text{FP}}{\text{FP} + \text{TN}}$
...
> Tip
>
> We can address class imbalance by further the F1 score across each class separately.
...
with $$i$$the true class, $$j$$the predicted class, and $$C_{i,k}$$the count of pixels with ground truth class $$i$$predicted by the model as class $$j$$.
